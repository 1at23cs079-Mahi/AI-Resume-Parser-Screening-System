...
def get_db_connection():
    ...
    if os.environ.get('VERCEL') == '1' or 'VERCEL' in os.environ:
        # Check if DB doesn't exist in /tmp, copy the pre-seeded version from git repo
        if not os.path.exists(Config.DATABASE):
            import shutil
            src_db = os.path.join(Config.BASE_DIR, 'database', 'recruitment_intelligence.db')
            if os.path.exists(src_db):
                print(f"Serverless deployment detected. Copying pre-seeded DB from {src_db} to {Config.DATABASE}...")
                os.makedirs(os.path.dirname(Config.DATABASE), exist_ok=True)
                shutil.copy2(src_db, Config.DATABASE)
            else:
                print(f"Pre-seeded database not found at {src_db}. Creating clean database...")
